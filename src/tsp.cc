          ...
          unsigned nthread = 1)
{
  ...
  auto run_one_thread = [&]() {
    ...
    // divide my number of threads so we use the same number of iterations
    // as single-threaded
    for (long i = 1; i <= iters/(pop_size * nthread); ++i) {
      ...
      // direct comparison here instead of is_improved() so we dont double print
      if (cities.total_path_distance(ordering) < best_dist) { 
        auto guard = std::scoped_lock(best_mutex);
        // we check again
  ...
  const auto diff = std::chrono::duration_cast<std::chrono::duration<double>>(end - start).count();
